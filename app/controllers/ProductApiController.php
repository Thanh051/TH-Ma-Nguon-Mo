...
class ProductApiController extends BaseApiController {
    ...
    // GET /api/product/detail/{id} hoặc /api/product/{id}
    ...
    public function create() {
        ...
        $ok = $this->productModel->create($name, $price, $image, $description, $category_id, $stock);
        $this->json([
            'status' => $ok,
            'message' => $ok ? 'Thêm sản phẩm bằng API thành công' : 'Không thể thêm sản phẩm'
        ], $ok ? 201 : 500);
    }
    ...
    public function update($id) {
        ...
        $ok = $this->productModel->update($id, $name, $price, $image, $description, $category_id, $stock);
        $this->json([
            'status' => $ok,
            'message' => $ok ? 'Cập nhật sản phẩm bằng API thành công' : 'Không thể cập nhật sản phẩm'
        ], $ok ? 200 : 500);
    }
    ...
    public function delete($id) {
        ...
        $this->json([
            'status' => $ok,
            'message' => $ok ? 'Xóa sản phẩm bằng API thành công' : 'Không thể xóa sản phẩm'
        ], $ok ? 200 : 500);
    }
}
